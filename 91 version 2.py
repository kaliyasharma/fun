...
PASSWORDS = {
    "JumboKing247@": "Launch playinjeet GUI",
    "admin123": r"C:\XboxGames\Asphalt Legends\Content\Asphalt9_gdk_x64_rtl.exe"  # Game path
}

# ---------------- USER CONFIG ----------------
EVENT_ID = "35105437"
EVENT_URL = f"https://playinjeet.com/mobile/eventDetails/{EVENT_ID}?type=2"
MAX_CONCURRENT = 10            # concurrency across users (not used yet)
SHOW_DEBUG_HTML = False        # save debug HTML files if True
# Bet delay is now configurable via GUI
# ---------------------------------------------

BASE_URL = "https://playinjeet.com"
...
class PasswordApp:
    ...
    def show_password_screen(self):
        ...
        instructions = """• JumboKing247@ - Launch Fun Test project
...
Note: Type exactly as shown above."""
    ...
    def check_password(self):
        ...
        if entered_password in PASSWORDS:
            ...
            if entered_password == "JumboKing247@":
                # Launch playinjeet GUI
                self.root.destroy()
                self.launch_playinjeet_gui()
    ...
    def launch_playinjeet_gui(self):
        """Launch the main playinjeet GUI."""
        main_root = tk.Tk()
...
class MainApp:
    def __init__(self, root):
        self.root = root
        root.title("Fun test Project GUI")
        ...
        # Middle: Bet Delay Settings
        settings_frame = tk.Frame(top_frm)
        ...
        # Right: Update Data Frame (NEW - replaces Volume Settings)
        update_data_frame = tk.Frame(top_frm)
        update_data_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(20, 10))
        ...
        # Right side: User Management buttons
        user_mgmt_frame = tk.Frame(top_frm)
        ...
        self.btn_load_users = tk.Button(user_mgmt_frame, text="📥 Load Users", 
                                       ...
                                       relief=tk.RAISED, bd=3,
                                       width=12)
        self.btn_load_users.pack(pady=(0, 5))
        ...
        self.save_logs_btn = tk.Button(user_mgmt_frame, text="💾 Save Logs", 
                                      ...
                                      relief=tk.RAISED, bd=2,
                                      width=12)
        self.save_logs_btn.pack(pady=5)
        ...
        enqueue_log("[GUI] Welcome to Fun Test project ")
...
9968071768,Alam1234"""
    ...
    def on_place_bet(self):
        ...
        if self.one_time_trigger_var.get():
            ...
            enqueue_log("[GUI] ⚡ ONE TIME TRIGGER ACTIVE - All bets will fire simultaneously!")
        else:
            try:
        ...
        async def placebet_flow():
            ...
            if bet_delay == 0:  # ONE TIME TRIGGER - All bets at once
                ...
                for i, w in enumerate(workers):
                    ...
                    self.pending_bet_tasks.append(task)  # Add to pending tasks list, also add a safety check to prevent too many concurrent requests
                max_concurrent = 20  # Limit concurrent requests to avoid overwhelming
                if len(bet_tasks) > max_concurrent:
                    enqueue_log(f"[GUI] Warning: {len(bet_tasks)} simultaneous bets may cause network issues")
                    enqueue_log(f"[GUI] Limiting to {max_concurrent} concurrent requests")
                    
                    # Process in batches
                    results = []
                    for i in range(0, len(bet_tasks), max_concurrent):
                        if self.betting_stopped:
                            break
                        batch = bet_tasks[i:i+max_concurrent]
